...
def dataset_builder(args, config):
    ...
    if args.distributed:
        ...
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=config.others.bs,
                                                 num_workers=1, #int(args.num_workers),
...
def build_opti_sche(base_model, config):
    ...
    param_groups = [
        ...
        {'params': finetune_head, 'weight_decay': weight_decay, 'lr': 10 * lr}
    ]

    if opti_config.type == 'AdamW':
        optimizer = optim.AdamW(param_groups, **opti_config.kwargs)
    elif opti_config.type == 'RMSprop':
        optimizer = optim.RMSprop(param_groups, **opti_config.kwargs)
