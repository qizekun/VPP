...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (data, _, _) in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            loss = base_model.module.training_grid_smoother(points)
            try:
                loss.backward()
            except:
                loss = loss.mean()
        ...
        if "validate" in config.keys():
            ...
            inf_config.model.smooth_config = config.model.smooth_config
            inf_config.model.smooth_config.ckpt_path = os.path.join(args.experiment_path, 'ckpt-last.pth')
